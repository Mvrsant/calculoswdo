...
def main():
    ...
    if menu == "📉 Paridades CME/BRLUSD":
        for ticker_key, nome in [("cme", "CME - 6L"), ("brl_usd", "BRL/USD")]:
            ...
            if df is not None:
                st.write(f"### {nome}")
                st.dataframe(estilizar_tabela(df, [f"Cotação ({nome})", "Valor Calculado"]), use_container_width=True)
        pass
    ...
    # Dados Carregados
        ...
        if dados_excel:
            st.dataframe(estilizar_tabela(pd.DataFrame([dados_excel]), list(dados_excel.keys()), cmap="Blues"), use_container_width=True)
        else:
    ...
    # Abertura Calculada
        ...
        tabela_metricas = pd.DataFrame({
            ...
            'Valor': [
                ...
                f"{wdo_abertura:.4f}" if wdo_abertura else "N/A",
                f"{dxy_variacao:.2f}%" if dxy_variacao else "N/A",
                f"{over:.4f}" if over else "N/A",
                f"{preco_justo:.4f}" if preco_justo else "N/A"
            ]
        })
        st.dataframe(estilizar_tabela(tabela_metricas, ["Valor"]), use_container_width=True)
        ...
        if all(x is not None for x in [wdo_abertura, over, sup_volb3]):
            ...
            st.dataframe(estilizar_bandas_ptax(df_bandas), use_container_width=True)
    ...
    # Cotações PTAX
        ...
        if qtde > 0:
            ...
            if tabela_bandas is not None:
                st.dataframe(estilizar_bandas_ptax(tabela_bandas), use_container_width=True)
